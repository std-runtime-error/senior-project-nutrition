...
export default function SimpleQuiz() {
  ...
  function getRandomQuestion() {
    if (usedQuestions.length == quiz.length) return quiz.length + 1;
    let x = getRandNumInRange();
    while (usedQuestions.includes(x)) x = getRandNumInRange();
  ...
  const [currentQuestion, setCurrentQuestion] = useState(
    getRandNumInRange(0, quiz.length)
  );
      ...
      {showScore ? (
        ...
        <div>
          <div className="question-section">
            <div className="question-count">Question {questionNumber + 1}</div>
          ...
          <div className="answer-section">
            {quiz[currentQuestion].answerList.map((answerOption) => (
              <button
                onClick={() =>
                  handleAnswerOptionClick(answerOption.checkCorrect)
                }
              >
                {answerOption.answer}
              </button>
